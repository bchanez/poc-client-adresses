...
| **Le piège** | `ClientLazyLoadingIT` | `findById` naïf → `LazyInitializationException` (l'adresse n'est **pas** là). `findByIdAvecAdresses` (join fetch) → tout chargé en **une seule** requête SQL (compteur Hibernate à l'appui, donc pas de N+1). |
...
L'échelle, ici, ce n'est pas le nombre d'adresses d'un client — c'est le **N+1** :
charger 1 000 clients puis leurs adresses une par une = 1 001 requêtes. `ClientLazyLoadingIT`
verrouille le `join fetch` (1 requête) : c'est ce qui garde la recherche tenable
quand la table grossit. (Recherche par `id` = clé primaire indexée : elle, elle
scale toute seule.)
